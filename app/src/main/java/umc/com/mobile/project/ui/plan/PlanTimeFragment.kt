...
class PlanTimeFragment : Fragment() {
	...
	private fun setupRecyclerView() {
		val adapter = PlanRecyclerAdapter(emptyList(), onAddButtonClicked = { timeResult ->
			if (timeResult != null) {
				viewModel.setSelectedTimeResult(
					timeResult = TimeResult(
						timeResult.searchGrade,
						timeResult.searchType,
						timeResult.searchName,
						timeResult.searchCredit
					)
				)
			}
		})
	...
	private fun observeViewModel() {
		...
		viewModel.listTimeInfo.observe(viewLifecycleOwner) { timeListResponse ->
			(binding.recyclerView.adapter as PlanRecyclerAdapter).updateTimeList(
				timeListResponse?.result ?: emptyList()
			)
		}
	}
