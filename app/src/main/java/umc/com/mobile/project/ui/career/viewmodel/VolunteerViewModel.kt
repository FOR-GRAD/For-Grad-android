...
class VolunteerViewModel : ViewModel() {
    ...
    fun searchVolunteerInfo(searchWord: String) {
        volunteerApiService.getSearchCareer("VOLUNTEERS", searchWord).enqueue(object :
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    if (CategoryListResponse != null) {
                        ...
                        Log.d("searchVolunteerInfo", "${response.body()}")
                    } else {
                        _error.postValue("서버 응답이 올바르지 않습니다.")
                    }
                } else {
                    _error.postValue("검색한 봉사 정보를 가져오지 못했습니다.")
                    try {
                        throw response.errorBody()?.string()?.let {
                            RuntimeException(it)
                        } ?: RuntimeException("Unknown error")
                    } catch (e: Exception) {
                        Log.e("searchVolunteerInfo", "searchVolunteerInfo API 오류: ${e.message}")
                    }
                }
            }
    ...
    fun getVolunteerInfo() {
        volunteerApiService.getCareerList("VOLUNTEERS")
